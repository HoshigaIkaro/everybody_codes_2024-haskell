...
mAddStar (mapping, currentDist) rest = mAddStar (newMapping, currentDist + extraDist) newRest
  where
    (point, closest, extraDist) = minimumBy (compare `on` (^. _3)) $ map (`minDistToStarInConst` mapping) rest
    f a = Just . maybe [a] (a :)
...
makeCluster (x : rest) = Just $ go [x] rest
  where
    go current remaining = case filter (\p -> any ($ p) distFuncs) remaining of
        [] -> (current, remaining)
        newStars -> go (newStars <> current) (remaining \\ newStars)
      where
        distFuncs = map (((< 6) .) . dist) current
...
part3 =
    product
        . take 3
        . sortOn negate
        . map (constellationSize . makeConstellationAndDist)
        . clusterStars
        . pStars