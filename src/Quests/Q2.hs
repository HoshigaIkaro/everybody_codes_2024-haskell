...
part1 :: Text -> Int
part1 s = sum $ map (`T.count` text) wordList
  where
    ls = T.lines s
    wordList = toWordList $ head ls
    text = ls !! 2
...
symbolsInLine wordlist line = go line 0
  where
    prefixwords str = filter (`T.isPrefixOf` str) wordlist
    bestword = maximumBy (compare `on` T.length) . prefixwords
    go :: Text -> Int -> Int
    go "" n = n
    go str 0
        | null (prefixwords str) = go (T.drop 1 str) 0
        | otherwise = succ . go (T.drop 1 str) . pred . T.length $ bestword str
    go str n
        | null (prefixwords str) = 1 + go (T.drop 1 str) (n - 1)
        | otherwise =
...
part2 s = sum . map handleLine $ drop 2 ls
  where
    ls = T.lines s
    wordlist = wordListAddReverse $ toWordList $ head ls
    handleLine = symbolsInLine wordlist
...
newWordPositionWithDir :: Point -> String -> Direction -> [PositionChar]
newWordPositionWithDir (row, col) word dir =
    case dir of
        UpDir -> zipWith (\char delta -> ((row - delta, col), char)) word [0 ..]
        DownDir -> zipWith (\char delta -> ((row + delta, col), char)) word [0 ..]
        LeftDir -> zipWith (\char delta -> ((row, col - delta), char)) word [0 ..]
        RightDir -> zipWith (\char delta -> ((row, col + delta), char)) word [0 ..]
...
allPositionVectorsForWord size point word = mapMaybe processDirection [UpDir, DownDir, LeftDir, RightDir]
  where
    processDirection = maybeWrappedDirections size . newWordPositionWithDir point word
...
part3 s = S.size $ processBoard board (height, width) wordlist
  where
    ls = T.lines s
    wordlist = map T.unpack . toWordList $ head ls
    boardLines = map T.unpack $ drop 2 ls
    height = length boardLines
    width = length $ head boardLines
    f (row, rowNum) =
        zipWith
            (\col colNum -> ((rowNum, colNum), col))
            row
            [0 ..]
