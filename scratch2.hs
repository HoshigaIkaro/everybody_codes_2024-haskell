...
updateCharAtWithUnknown start@(topmostRow, leftmostCol) point@(rowNum, colNum) maybeBoard = do
    ...
    row <- getRowWithFilter filterFunc topmostRow rowNum board
    col <- getColWithFilter filterFunc leftmostCol colNum board
...
processInOrder (p : ps) (solvedStartingPoints, board) =
    let prepared = partialSolve p (runePositions p) board
     in case maybeCompleteSolve p prepared of
...
part3 s = sum $ map (effectivePower . flip readRunic finalBoard . runePositions) startingPoints
  where
    ...
    (startingPoints, finalBoard) = processInOrder points ([], board)
